...
def GetItem(Items, ItemToGet, CurrentLocation):
  ...
  if IndexOfItem == -1:
    ...
    print("You can't find " + ItemToGet + ".")
  elif len([item for item in Items if item.Location == INVENTORY]) >= 5:
    print("You already have max num of items")
  else:
    CanGet = True
  if CanGet:
...
def PlayGame(Characters, Items, Places):
  ...
  while not StopGame:
    ...
    if Command == "get":
      ...
      drop_item(Items, Instruction, Characters[0].CurrentLocation)
    elif Command == "quit":
      Say("You decide to give up, try again another time.")
      StopGame = True
    else:
      print(random_invalid_message_for(Command))
  input()

def drop_item(items, itemToGet, current_location):
  inventory_items = [item for item in items if item.Location == INVENTORY]
  index = GetIndexOfItem(itemToGet, -1, inventory_items)
  if index != -1:
    item = inventory_items[index]
    if "fragile" in item.Status:
      items.remove(item)
      print("Oops, item broke when dropped!")
    else:
      item.Location = current_location
      print("item has been dropped at location: " + str(current_location))
  else:
    print("sorry, item not in inventory")

  
def random_invalid_message_for(command):
  if random.randint(0,1) == 1:
    return "Sorry, you don't know how to " + command + "."
  else:
    return "Sorry, I don't know what " + command + "means."
